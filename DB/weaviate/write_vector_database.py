...
client.collections.create(
    name="Documents",
    properties=[
        Property(name="metin", data_type=DataType.TEXT),
        Property(name="chunk_id", data_type=DataType.INT),
        Property(name="doc_id", data_type=DataType.INT),
        Property(name="filename", data_type=DataType.TEXT),
        Property(name="filepath", data_type=DataType.TEXT),
    ],
    vectorizer_config=Configure.Vectorizer.none(),  # Manuel embedding kullanacağız
)
...
print(f"\n{'='*60}")
print(f"✓ {len(tum_veriler)} adet veri başarıyla Weaviate'e kaydedildi!")
...
print(f"   Toplam kayıt: {agg.total_count}")

client.close()
print("✓ Weaviate bağlantısı kapatıldı")